...
class GP:
    ...
    def __init__(self, X, Z):
        ...
        Args: 
            ...
            Z: n x m array, normalized rssi readings with floats ranging from 0 to 1
        """
        self.set_data(X, Z)
    ...
    def set_parameters(self, epsilon, penalty_factor, number_of_var):
        """
    ...
    def calculate_pathloss_value(self, x, parameters, epsilon, verbose=False):
        """
    ...
    def calculate_pathloss_model(self):
        """
        Calculate path loss parameters for each access point.
        """
        for i in range(self.m_):
            arg_list = (i, self.X_, self.Z_, self.epsilon_, self.penalty_factor_, self.calculate_pathloss_value)
    ...
    def calculate_GP(self):
        ...
        for i in range(self.m_):
            ...
            pl = self.calculate_pathloss_value(self.X_, parameter, self.Z_[:, i], self.epsilon_)
    ...
    def predict(self, x):
        ...
        for i in range(self.m_):
            pl = self.calculate_pathloss_value(x, self.path_loss_params_[:, i], self.epsilon_)
...
if __name__ == "__main__":
    ...
    pl = solver.calculate_pathloss_value(solver.X_, solver.path_loss_params_[:, 0], solver.epsilon_, True)
    sign = Z[:, 0] > 0
    print("Rssi vector:\n", Z[:, 0])
    print("Sign vector:\n", sign)
    ...
    solver.calculate_pathloss_model()
    solver.calculate_GP()
